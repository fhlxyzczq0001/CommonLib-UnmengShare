package com.app.common.HttpRequest.Interceptor;
 ...
 */
public class RequestEncapsulationInterceptor implements Interceptor{
    ...
    public Response intercept(Chain chain) throws IOException
    {
        ...
        //给请求设置Host
        String httpdnsMapKey=request.url().toString().substring(0, ToolsUtils.getCharacterPosition(request.url().toString(),"/",3));
        if(null!= CommonApplication.httpdnsHosMap.get(httpdnsMapKey)&&!CommonApplication.httpdnsHosMap.get(httpdnsMapKey).isEmpty()){
            newHeaderBuilder.header("Host", CommonApplication.httpdnsHosMap.get(httpdnsMapKey));
        }else {
    ...
    private void saveCookie(Headers head,String keyHost){
        ...
        if(null!=set_cookie&&!set_cookie.isEmpty()){
            CookieMapBean setCookie_MapBean= Common_SharePer_UserInfo.sharePre_GetCookieCache();
            if(setCookie_MapBean==null){
                setCookie_MapBean=new CookieMapBean();
