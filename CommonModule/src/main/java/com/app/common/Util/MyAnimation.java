...
public class MyAnimation {
     ...
     * @return
     */
    public static Animation shakeReverseAnimation(int counts) {
        Animation translateAnimation = new TranslateAnimation(0, 10, 0, 0);
        translateAnimation.setInterpolator(new CycleInterpolator(counts));
    ...
    public static Animation rotateClockwiseAnimation(Context context) {
        Animation rotate = AnimationUtils.loadAnimation(context, R.anim.rotate_clockwise);//创建顺时针旋转180度动画
    ...
    public static Animation rotateCounterclockwiseAnimation(Context context) {
        Animation rotate = AnimationUtils.loadAnimation(context, R.anim.rotate_counterclockwise);//创建顺时针旋转180度动画
        rotate.setInterpolator(new LinearInterpolator());//设置为线性旋转
        rotate.setFillAfter(!rotate.getFillAfter());//每次都取相反值，使得可以不恢复原位的旋转
        return rotate;
    }
